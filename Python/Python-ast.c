...
static int astmodule_clear(PyObject *module)
{
    ...
    Py_CLEAR(astmodulestate(module)->Sandbox_type);
...
static int astmodule_traverse(PyObject *module, visitproc visit, void* arg)
{
    ...
    Py_VISIT(astmodulestate(module)->Sandbox_type);
...
obj2ast_stmt(PyObject* obj, stmt_ty* out, PyArena* arena)
{
    ...
    tp = astmodulestate_global->Sandbox_type;
    isinstance = PyObject_IsInstance(obj, tp);
    if (isinstance == -1) {
        return 1;
    }
...
PyInit__ast(void)
{
    ...
    if (PyModule_AddObject(m, "Sandbox", astmodulestate_global->Sandbox_type) <
        0) {
        goto error;
    }
    Py_INCREF(astmodulestate(m)->Sandbox_type);
